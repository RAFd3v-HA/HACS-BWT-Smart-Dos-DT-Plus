...
class BWTDataCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    """Coordinator that fetches dynamic values every 120 seconds and static values once."""
    ...
    async def async_load_static_data(self) -> None:
        """Load less dynamic/static data once per integration start."""
        if self.static_data:
            return
